...
for e in EAAs:
    ...
    for ext in [ext1, ext2]:
        ...
        plt.plot(avg_l, '--', color=EAA_colors[ext][e], label=ext + e)
        plt.scatter(x, [avg_l[i] for i in range(0, len(avg_l), plot_freq)], s=30, marker='o', c=EAA_colors[ext][e])
        plt.fill_between(x, y1, y2, color=EAA_colors[ext][e], alpha=0.2)
