...
# state importance
libs.SarsaAgent_computeStateImportance.argtypes=[c_void_p,POINTER(c_double)]
libs.SarsaAgent_computeStateImportance.restype=c_double
...
class SarsaAgent(object):
  ...
  def computeStateImportance(self,state):
    s = (c_double * len(state))(*state)
    state_importance=libs.SarsaAgent_computeStateImportance(c_void_p(self.obj),s)
    # print ('state_importance', state_importance)
    return float(state_importance)

  def singleSaveWeights(self):
    libs.SarsaAgent_singleSaveWeights(c_void_p(self.obj))

  def singleLoadWeights(self):
    libs.SarsaAgent_singleLoadWeights(c_void_p(self.obj))
