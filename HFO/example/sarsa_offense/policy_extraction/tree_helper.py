...
"Modified for T-S Project"

def train_decision_tree_policy(D_prime, NA):
    clf = tree.DecisionTreeClassifier()
    X = [D_prime[i][0] for i in range(len(D_prime))]
    Y = [D_prime[i][1] for i in range(len(D_prime))]

    # ensure the dimension of input is correct
    # each action has a representative
    for a in range(NA):
        X.append(np.random.random(len(D_prime[0][0])))
        Y.append(a)
...
def collect_clf_paths(paths, printline=False):
    node_dict = {}
